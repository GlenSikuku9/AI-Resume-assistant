...
function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch total resumes
      const totalResumesResponse = await fetch(
        `${import.meta.env.VITE_API_URL}/api/admin/total-resumes`, 
        { headers }
      );

      const totalResumesText = await totalResumesResponse.text();

      // Try parsing JSON only if content-type is JSON
      let totalResumesData = {};
      if (totalResumesResponse.headers.get("content-type")?.includes("application/json")) {
        totalResumesData = JSON.parse(totalResumesText);
      } else {
      ...
      const analyticsResponse = await fetch(
        `${import.meta.env.VITE_API_URL}/api/admin/admin-analytics`,
        { headers }
      );
      ...
      let analyticsData = {};
      if (analyticsResponse.headers.get("content-type")?.includes("application/json")) {
      ...
      const templatesRes = await fetch(`${import.meta.env.VITE_API_URL}/api/templates`);
      const templatesText = await templatesRes.text();

      let templates = [];
      if (templatesRes.headers.get("content-type")?.includes("application/json")) {
